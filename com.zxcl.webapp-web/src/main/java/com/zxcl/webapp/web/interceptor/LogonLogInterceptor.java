...
@Component
public class LogonLogInterceptor implements HandlerInterceptor {
	...
	private void insertLogLoginInfo(String userId, boolean isMobile, String SID , String userAgent){
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
		...
		try {
			...
			LogLoginInfo logLoginInfo = logLoginInfoDAO.selectNearRecordByUserId(userId);
			if(logLoginInfo != null){
				
				//相同session
				if(SID.equals(logLoginInfo.getSid())){
					
					//不同终端
					if(logLoginInfo.getIsMobile() != (isMobile?1:0) || !logLoginInfo.getUserAgent().equals(userAgent)){
						insertLogLoginInfo(userId, isMobile, SID, userAgent);
					}
				}
			...
			logger.info("登录后终端记录失败", e);
		}
		logger.info("登录后终端记录结束");
		return true;
	}
