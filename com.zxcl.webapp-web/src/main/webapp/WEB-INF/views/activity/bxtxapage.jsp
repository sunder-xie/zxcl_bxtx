...
<body>
	...
	<div class="swiper-container">
		<div class="swiper-wrapper">
			<div onclick="now()" style="width:46%;height:9%;position: absolute;top:44%;left:29%;"></div>
			<div class="swiper-slide">
				<img src="${ctx}activity/bxtxa/bxtxa01.jpg">
			</div>
...
<div class="wrap_result load" style="display:none;">
	...
	<script type="text/javascript">
		...
		function now(){
			$(".load").show();
			$.post("${ctx}activity/bxtxaOpenRedPacket.do",{},function(data){
				
				$(".load").hide();
				if(data.status > 0){
					if(data.prizeType == "1"){
						$("#moneye").html(data.amount);
						$("#all").show();
						$("#cj").show();
						$("#cd").hide();
					}else{
						alert("恭喜领取到礼品：" + data.prize);
					}
					

				}else if(data.status == -1 ){
					alert("哎呀，红包太重，没有抽出来，请重新打开。");
				}else if(data.status == -10 || data.status == -11 || data.status == -12 ||
						data.status == -13 || data.status == -14){
					alert("领取红包时间还没开始哟..休息休息再来吧.");
				}else if(data.status == -15 ){
					alert("本周努力，下周领奖，加油！");
				}else if(data.status == -16 ){
					alert("您本周已抽奖,下周继续努力.");
				}else if(data.status == -17 ){
					alert("红包已被领取,请到钱包查看明细.");
				}else{
					alert("哎呀，红包太重，没有抽出来，请重新打开。");
				}
			});
