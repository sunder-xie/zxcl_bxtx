...
<link rel="stylesheet" type="text/css" href="${ctx}css/more.css">
...
<div class="wrap" style="margin-bottom: 44%;">
	<section  >
		<ul class="moreList"  >
			<li><a href="${ctx}invite.do">推荐好友</a></li>
...
<div class="maskBox" style="background: rgba(0, 0, 0, .35);">
	<div class="comfirm">
		<p class="notic"><img src="${ctx}images/weixin.png"><span id="bind_text">绑定微信免登录</span></p>
		<div class="btns">
...
function check_wechat_bind(){
	$.post('${ctx}check_wechat_bind.do',{},function(data){
		if(data.success){
			if(data.data == 0){
				$('#bind_text').text('绑定微信免登录');
				$('#bind').text('暂不绑定');
				$('#unBind').attr('onclick','bd()').text('绑定');
				$('.maskBox').show();
			}else{
				$('#bind_text').text(data.message);
				$('#bind').text('确定');
				$('#unBind').attr('onclick','remove_bind()').text('解除绑定');
				$('.maskBox').show();
			}
		}else{
...
function remove_bind(){
	$('.maskBox').hide();
	$.post('${ctx}remove_bind_wechat.do',{},function(data){
		$('#resultInfo').text(data.message);
		$("#select_result").show();
	},'json');
}
